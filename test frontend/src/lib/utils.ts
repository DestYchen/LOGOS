...
// Generates a reasonably unique ID in all environments.
// Prefers Web Crypto `randomUUID`, falls back to `getRandomValues`, then time+random.
export function safeRandomId(): string {
  try {
    // In modern browsers on secure contexts
  ...
  try {
    // Use getRandomValues to form a RFC4122 v4-like string
  ...
  // Last resort: time + random string
